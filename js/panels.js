...
(function () {
  ...
  Panels.App = Backbone.Router.extend({
      ...
      if (!loc) {
        ...
        if (rightPanel)
          leftPanel.whenLoaded(function () {
            var source = leftPanel.$(
              'a[href="' + this.root + rightPanel.fragment + '"]',
            );
    ...
    calculateLayout: function (loc, limit1) {
      ...
      do {
        ...
        if (numPanels && curWidth + width > maxWidth - (loc ? goLeftWidth : 0))
          break;
      ...
      for (var i = loc + 1, max = this.targetI; i <= max; i++) {
        ...
        if (hasSidebar && i == loc + 1) {
          ...
          var targetWidth = Math.floor(
            panel.minWidth + bufferRatio * (panel.maxWidth - panel.minWidth),
          );
          if (
            panel.targetWidth !== undefined &&
    ...
    getPanelWidth: function (panel) {
      if (panel.width === "auto")
        return this.targetWidth - panel.left - panel.right;
      return panel.width;
    },
    commitLayout: function (instant) {
      var goingBack =
        this.targetI < this.i ||
        (this.targetI === this.i && this.targetJ < this.j);
      if (goingBack) {
        // <--
        var totalWidth = this.targetWidth - this.targetRightGap;
        for (
          var i = Math.max(this.j, this.targetI + 1), end = this.i;
          i <= end;
          i++
        ) {
        ...
        for (
          var i = Math.min(this.j - 1, this.targetI), end = this.targetJ;
          i >= end;
          i--
        ) {
        ...
        for (
          var i = Math.min(this.i, this.targetJ - 1), end = this.j;
          i >= end;
          i--
        ) {
        ...
        for (
          var i = Math.max(this.i + 1, this.targetJ), end = this.targetI;
          i <= end;
          i++
        ) {
  ...
  var Panel = (Panels.Panel = View.extend({
    constructor: function (options) {
      ...
      if (!this.events["click a, button"])
        this.events["click a, button"] = "handleNavigation";
      if (!this.events["submit form"])
        this.events["submit form"] = "handleNavigation";
      ...
      if (this.source) {
        ...
        if (this.source[0].tagName.toUpperCase() === "BUTTON")
          this.source[0].disabled = true;
      }
    ...
    moveTo: function (loc, flags, instant) {
      ...
      if (instant) {
        ...
        if (flags.startingOffset !== undefined) {
          ...
          this.$el.css(
            "width",
            $("body").width() - oldLeft - oldRight + this.widthOffset,
          );
        } else if (right !== "auto" && oldRight === "auto") {
          this.$el.css(
            "right",
            $("body").width() - oldLeft - oldWidth + this.rightOffset,
          );
        }
        this.$el.transition(
          {
    ...
    updateBackButton: function () {
      if (this.sourcePanel) {
        if (this.sourcePanel.shortTitle) {
          this.$(".pfx-backbutton").html(
            this.app.backButtonPrefix +
              this.sourcePanel.shortTitle.replace(/</g, "&lt;"),
          );
        }
    ...
    handleNavigation: function (e) {
      ...
      var linkHref =
        target.data("href") || target.attr("href") || target.attr("action");
      var formMethod = "";
      if (target[0].tagName.toUpperCase() === "FORM") {
        formMethod = "GET";
        if ((target.attr("method") || "").toUpperCase() === "POST")
          formMethod = "POST";
        target
